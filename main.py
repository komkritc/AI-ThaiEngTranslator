...
Usage:
    ...
    CLI Mode:   python translate_docx.py --input proposal_thai.docx --output proposal_english.docx --direction thai-to-english
"""
...
class RateLimiter:
    ...
    def __init__(self, requests_per_second: int = 5, requests_per_minute: int = 200):
        ...
        self.min_interval_per_minute = 60.0 / requests_per_minute if requests_per_minute > 0 else 0.3
...
class OpenRouterClient(BaseLLMClient):
    ...
    def chat_completion(self, model: str, system_prompt: str, user_text: str, stats: Dict[str, int]) -> str:
        ...
        try:
            ...
            if "400" in error_msg:
                logger.error(f"Bad request - model '{model}' may not exist or is not accessible")
...
class OllamaClient(BaseLLMClient):
    ...
    def chat_completion(self, model: str, system_prompt: str, user_text: str, stats: Dict[str, int]) -> str:
        ...
        try:
            ...
            stats["failures"] = stats.get("failures", 0) + 1
            if "404" in str(e):
                logger.error(f"Model '{model}' not found in Ollama. Please pull it first:")
...
class TyphoonClient(BaseLLMClient):
    ...
    def __init__(self, api_key: str):
        if not TYPHOON_AVAILABLE:
            raise ImportError("OpenAI package is required for Typhoon. Install with: pip install openai")
        ...
        while retries < max_retries:
            try:
                ...
                if "authentication" in error_msg.lower() or "api_key" in error_msg.lower():
                    logger.error(f"Authentication failed - please check your Typhoon API key")
                    raise e
                elif "model" in error_msg.lower() and "not found" in error_msg.lower():
                    logger.error(f"Model '{model}' not found. Try 'typhoon-v2.5-30b-a3b-instruct'")
                    raise e
                else:
                    logger.warning(f"API error (attempt {retries}/{max_retries}): {e}")
...
def translate_text(client: BaseLLMClient, text: str, model: str, direction: str, stats: Dict[str, int]) -> str:
    ...
    system_prompt = TRANSLATION_PROMPTS.get(direction, TRANSLATION_PROMPTS["thai-to-english"])
    language_prefix = LANGUAGE_PREFIXES.get(direction, LANGUAGE_PREFIXES["thai-to-english"])
    
    results = []
    total_chunks = len(chunks)
    
    for i, chunk in enumerate(chunks):
        logger.debug(f"Translation chunk {i+1}/{total_chunks}: {len(chunk)} chars")
        # Add language direction prefix to the user text
        user_text_with_prefix = f"{language_prefix}{chunk}"
        res = client.chat_completion(model, system_prompt, user_text_with_prefix, stats)
        results.append(res)
...
def run_translation_task(
    ...
    progress_callback: Callable[[int, int, str], None] = None,  # (current, total, status_text)
    ...
    try:
        if api_type.lower() == "openrouter":
            ...
            logger.info("Typhoon rate limiting enabled (5 req/sec, 200 req/min)")
        else:
            client = create_llm_client(
                api_type="ollama",
                base_url=ollama_url
    ...
    # Estimate time
    est_seconds, est_time_str = estimate_processing_time(total_chunks, api_type, model)
    logger.info(f"Estimated processing time: {est_time_str} ({total_chunks} chunks)")
    
    if progress_callback:
        progress_callback(0, total_chunks, f"Estimating... {est_time_str} expected")
    
    # Progress tracker
    progress = TranslationProgress(total_chunks)
    progress.start()
    
    logger.info(f"Found {total_segments} segments, {total_chunks} chunks requiring translation.")
    ...
    for i, (p, p_type) in enumerate(to_process):
        ...
        try:
            ...
            if translated_text and translated_text != text:
                ...
                logger.warning(
                    f"Translation returned empty or unchanged for segment {i+1}")
...
class TranslatorGUI(ctk.CTk):
    def __init__(self):
        ...
        self.title("Desinged by Komkrit Chooruang")
    ...
    def build_ui(self):
        ...
        ctk.CTkLabel(left_panel, text="Output DOCX:").pack(padx=20, anchor="w")
        ...
        # API Type
        self.api_type_var = ctk.StringVar(value="Ollama")
        ...
        ctk.CTkLabel(left_panel, text="Translation Direction:").pack(padx=20, anchor="w")
        ...
        # Model
        self.model_var = ctk.StringVar(value="scb10x/typhoon-translate1.5-4b:latest")
        ctk.CTkLabel(left_panel, text="Model Name:").pack(padx=20, anchor="w")
        self.model_entry = ctk.CTkEntry(left_panel, textvariable=self.model_var)
        ...
        self.ollama_url_frame = ctk.CTkFrame(left_panel, fg_color="transparent")
        
        ctk.CTkLabel(self.ollama_url_frame, text="Ollama Base URL:").pack(anchor="w")
        ...
        if input_path:
            ...
            if direction == "Thai to English":
                self.output_var.set(os.path.join(
                    os.path.dirname(input_path), f"{name}_english{ext}"))
            else:
                self.output_var.set(os.path.join(
                    os.path.dirname(input_path), f"{name}_thai{ext}"))
    ...
    def browse_input(self):
        ...
        if filepath:
            ...
            if direction == "Thai to English":
                self.output_var.set(os.path.join(
                    os.path.dirname(filepath), f"{name}_english{ext}"))
            else:
                self.output_var.set(os.path.join(
                    os.path.dirname(filepath), f"{name}_thai{ext}"))
    ...
    def load_env(self):
        ...
        if key:
            ...
            messagebox.showinfo("Success", f"API Key loaded from .env for {api_type}")
        else:
    ...
    def start_translation(self):
        ...
        if api_type.lower() in ["openrouter", "typhoon"]:
            ...
            if not api_key:
                messagebox.showerror("Error", f"Please provide an {api_type} API key.")
                return
        else:
            ollama_url = self.ollama_url_var.get()
            if not ollama_url:
                messagebox.showerror("Error", "Please provide an Ollama base URL.")
                return
        ...
        self.worker_thread = threading.Thread(
            ...
            args=(input_path, output_path, model, api_type, direction_key, api_key, ollama_url),
            daemon=True
        )
        self.worker_thread.start()

    def run_worker(self, input_path, output_path, model, api_type, direction, api_key, ollama_url):
        try:
            stats = run_translation_task(
                ...
                progress_callback=lambda c, t, status: self.progress_queue.put((c, t, status)),
...
def main():
    ...
    parser.add_argument(
        "--model",
        default="scb10x/typhoon-translate1.5-4b:latest",
        help="Model name."
    )
    parser.add_argument(
        "--api-type",
